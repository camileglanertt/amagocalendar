...
eventSources: [
 ...
 // any other sources...
   ...
   eventClick: function(info) {
       ...
       var mes = info.event.start.getMonth();
       mes = mes+1;
       if(mes < 10){
         mes = '0'+mes;
       }
       ...
       var minutos = info.event.start.getMinutes();
 
       Swal.fire({
   title: 'Atendimento n°'+ info.event.id,
   html: "Cliente: " + info.event.title + "<br>"+ "Data: "+dia+"/"+mes+"/"+ano+"<br>Horário: "+hora+"h"+ minutos +"min"+ "<br>"+"CPF Cliente: " + info.event.extendedProps.cpfCliente + "<br>Telefone Cliente: " + info.event.extendedProps.telefoneCliente + "<br>E-mail Cliente: " + info.event.extendedProps.emailCliente + "<br><br><a class='btn btn-danger' href='calendario.php?delete=1&code="+info.event.id+"&date="+dia+"/"+mes+"/"+ano+"'> Cancelar atendimento </a>",
   type: 'info',
   showCancelButton:true,
   cancelButtonText: 'Enviar e-mail',
 }).then((result) => {
   if (result.dismiss === Swal.DismissReason.cancel) {
     Swal.fire({
       title: "Enviar e-mail para " + info.event.extendedProps.emailCliente,
       html: "Mensagem:",
       input: 'text'
 
     }).then((result) => {
       setTimeout(function(){ window.location.href = "inc/sendmail.inc.php?email="+info.event.extendedProps.emailCliente+ "&mensagem="+ result.value}, 1000);
